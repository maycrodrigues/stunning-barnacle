...
export const UrgencySettings: React.FC = () => {
  const { urgencyOptions, addUrgency, removeUrgency, updateUrgency, reorderUrgencies } = useAppStore();
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = urgencyOptions.findIndex((item) => item.value === active.id);
      const newIndex = urgencyOptions.findIndex((item) => item.value === over.id);
      reorderUrgencies(arrayMove(urgencyOptions, oldIndex, newIndex));
    }
  };
  ...
  const handleAdd = () => {
    ...
    const value = generateSlug(newUrgency);

    const option: Option = { value, label: newUrgency };
    try {
        addUrgency(option);
        setNewUrgency("");
    } catch (error: any) {
  ...
  const handleEditStart = (option: Option) => {
    setEditingUrgency(option.value);
  ...
  const handleEditSave = (originalValue: string) => {
    ...
    try {
        updateUrgency(originalValue, editLabel, formattedSlug);
        setEditingUrgency(null);
        setEditLabel("");
        setEditSlug("");
        setSlugEditedManually(false);
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Adicione, edite e reordene os níveis de urgência do sistema.
            </p>
          </div>
          <div className="rounded-full bg-brand-50 px-3 py-1 text-xs font-medium text-brand-600 dark:bg-brand-900/20 dark:text-brand-400">
            {urgencyOptions.length} urgências
          </div>
        </div>
        ...
        <div className="mb-8 flex gap-3">
          <div className="relative flex-1">
            <Input
              placeholder="Nome da nova urgência..."
              value={newUrgency}
        ...
        <div className="space-y-3">
          ...
          >
            <SortableContext
              items={urgencyOptions.map((opt) => opt.value)}
          ...
          {urgencyOptions.length === 0 && (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 py-12 dark:border-gray-700 dark:bg-gray-800/50">
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Comece adicionando uma nova urgência acima.
              </p>
            </div>
          )}
        </div>
