...
export const PoliticalSpectrumSettings: React.FC = () => {
  const { politicalSpectrumOptions, addPoliticalSpectrum, removePoliticalSpectrum, updatePoliticalSpectrum, reorderPoliticalSpectrums } = useAppStore();
  const [newSpectrum, setNewSpectrum] = useState("");
  const [editingSpectrum, setEditingSpectrum] = useState<string | null>(null);
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = politicalSpectrumOptions.findIndex((item) => item.value === active.id);
      const newIndex = politicalSpectrumOptions.findIndex((item) => item.value === over.id);
      reorderPoliticalSpectrums(arrayMove(politicalSpectrumOptions, oldIndex, newIndex));
    }
  };

  const handleAdd = () => {
    if (!newSpectrum.trim()) return;
    
    const value = generateSlug(newSpectrum);

    const option: Option = { value, label: newSpectrum };
    try {
  ...
  const handleEditStart = (option: Option) => {
    setEditingSpectrum(option.value);
  ...
  const handleEditSave = (originalValue: string) => {
    ...
    try {
        updatePoliticalSpectrum(originalValue, editLabel, formattedSlug);
        setEditingSpectrum(null);
        setEditLabel("");
        setEditSlug("");
        setSlugEditedManually(false);
  ...
  const handleRemove = (value: string) => {
    Swal.fire({
      ...
      if (result.isConfirmed) {
        ...
        Swal.fire({
          title: "Excluído!",
          text: "O espectro político foi removido.",
          icon: "success",
          showConfirmButton: false,
          timer: 2000,
        });
      ...
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div className="mb-6 flex items-center justify-between">
          <div>
            <h3 className="text-lg font-semibold text-gray-900 dark:text-white">
              Gerenciar Espectro Político
            </h3>
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Adicione, edite e reordene as opções de espectro político.
            </p>
          </div>
          <div className="rounded-full bg-brand-50 px-3 py-1 text-xs font-medium text-brand-600 dark:bg-brand-900/20 dark:text-brand-400">
            {politicalSpectrumOptions.length} opções
          </div>
        </div>
        ...
        <div className="mb-8 flex gap-3">
          <div className="relative flex-1">
            <Input
              placeholder="Nome do novo espectro..."
              value={newSpectrum}
              onChange={(e) => setNewSpectrum(e.target.value)}
              className="pl-4 pr-4"
        ...
        <div className="space-y-3">
          ...
          >
            <SortableContext
              items={politicalSpectrumOptions.map((opt) => opt.value)}
              strategy={verticalListSortingStrategy}
            >
              {politicalSpectrumOptions.map((option) => (
          ...
          {politicalSpectrumOptions.length === 0 && (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 py-12 dark:border-gray-700 dark:bg-gray-800/50">
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Comece adicionando uma nova opção acima.
              </p>
            </div>
          )}
        </div>
