...
export const RoleSettings: React.FC = () => {
  const { roleOptions, addRole, removeRole, updateRole, reorderRoles } = useAppStore();
  ...
  const handleDragEnd = (event: DragEndEvent) => {
    ...
    if (over && active.id !== over.id) {
      const oldIndex = roleOptions.findIndex((item) => item.value === active.id);
      const newIndex = roleOptions.findIndex((item) => item.value === over.id);
      reorderRoles(arrayMove(roleOptions, oldIndex, newIndex));
    }
  };
  ...
  const handleAdd = () => {
    ...
    const value = generateSlug(newRole);

    const option: Option = { value, label: newRole };
    try {
      addRole(option);
      setNewRole("");
    } catch (error: any) {
  ...
  const handleEditSave = (originalValue: string) => {
    ...
    try {
        updateRole(originalValue, editLabel, formattedSlug);
        setEditingRole(null);
        setEditLabel("");
        setEditSlug("");
        setSlugEditedManually(false);
    ...
    <div className="space-y-6">
      <div className="rounded-xl border border-gray-200 bg-white p-6 shadow-sm dark:border-gray-800 dark:bg-gray-900">
        <div className="mb-6 flex items-center justify-between">
          <div>
            ...
            <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
              Adicione, edite e reordene os cargos da equipe.
            </p>
          </div>
        ...
        <div className="space-y-3">
          ...
          >
            <SortableContext
              items={roleOptions.map((opt) => opt.value)}
          ...
          {roleOptions.length === 0 && (
            <div className="flex flex-col items-center justify-center rounded-xl border border-dashed border-gray-300 bg-gray-50 py-12 dark:border-gray-700 dark:bg-gray-800/50">
              ...
              <p className="mt-1 text-sm text-gray-500 dark:text-gray-400">
                Comece adicionando um novo cargo acima.
              </p>
            </div>
          )}
        </div>
